...
class NPlusOneAnalyzer:
    ...
    def deduplicate_queries(self, query_calls: List[ast.AST], loop_node: ast.AST) -> List[ast.AST]:
        """
        Deduplicate query calls based on their source representation and exact location in the code.
        """
        unique_calls = {}
        for call in query_calls:
            source = ast.get_source_segment(self.source_code, call)
            loop_start_line = getattr(loop_node, 'lineno', 0)
            call_line = getattr(call, 'lineno', 0)
            key = f"{source}-{loop_start_line}-{call_line}"
